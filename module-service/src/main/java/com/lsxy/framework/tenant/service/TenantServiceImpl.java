...
public class TenantServiceImpl extends AbstractService<Tenant> implements TenantService {
    ...
    public Map getAwaitNum() {
        String hql = " FORM Tenant obj where obj.isRealAuth=?1 or obj.isRealAuth=?2 ";
        long tenant =  countByCustom(hql,true,Tenant.AUTH_WAIT,Tenant.AUTH_ONESELF_WAIT);
        String hql2 = "  FORM VoiceFilePlay obj where obj.status=?1  ";
        long voiceFilePlay = countByCustom(hql2,true, VoiceFilePlay.STATUS_WAIT);
        Map map = new HashMap();
