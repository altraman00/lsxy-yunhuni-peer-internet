...
public class ApiCallDayServiceImpl extends AbstractService<ApiCallDay> implements ApiCallDayService {
    ...
    public void dayStatistics(Date date1, int hour1,Date date2,int hour2,String[] select) throws  SQLException{
        ...
        String sql = " insert into db_lsxy_base.tb_base_api_call_day("+selects+"dt,day,among_api,sum_api,create_time,last_time,deleted,sortno,version ) " +
                " select "+selects+" ? as dt,? as day, "+
                " count(1) as among_api, " +
                " count(1)+IFNULL((select sum_api from db_lsxy_base.tb_base_api_call_day h where "+wheres+" h.dt = ? and h.day=? ),0) as  sum_api, " +
                " ? as create_time,? as last_time,? as deleted,? as sortno,? as version ";
        if(StringUtil.isNotEmpty(groupbys)){
            groupbys = " group by "+groupbys;
        }
        sql += " from db_lsxy_base.tb_base_api_call_hour a where tenant_id is not null and app_id is not null and type is not null and a.dt>=? and a.dt<=? "+groupbys;
