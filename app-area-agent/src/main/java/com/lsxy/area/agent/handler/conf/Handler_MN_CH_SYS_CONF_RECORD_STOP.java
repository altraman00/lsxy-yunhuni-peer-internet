...
public class Handler_MN_CH_SYS_CONF_RECORD_STOP extends RpcRequestHandler{
    ...
    public RPCResponse handle(RPCRequest request, Session session) {
        ...
        try {
            cticlient.operateResource(0, 0, res_id,"sys.conf.record_stop", params, new RpcResultListener(){

                @Override
                protected void onResult(Object o) {
                    if(logger.isDebugEnabled()){
                        logger.debug("调用sys.conf.record_stop成功conf_id={},result={}",conf_id,o);
                    }
                }

                @Override
                protected void onError(RpcError rpcError) {
                    logger.error("调用sys.conf.record_stop失败conf_id={},result={}",conf_id,rpcError);
                }

                @Override
                protected void onTimeout() {
                    logger.error("调用sys.conf.record_stop超时conf_id={}",conf_id);
                }
            });
