...
public class Handler_MN_CH_SYS_CALL_CONF_EXIT extends RpcRequestHandler{
    ...
    public RPCResponse handle(RPCRequest request, Session session) {
        ...
        try {
            cticlient.operateResource(new BusAddress((byte)0,(byte)0), res_id,"sys.call.conf_exit", params, new RpcResultListener(){

                @Override
                protected void onResult(Object o) {
                    if(logger.isDebugEnabled()){
                        logger.debug("调用sys.call.conf_exit成功call_id={},result={}",call_id,o);
                    }
                }

                @Override
                protected void onError(RpcError rpcError) {
                    logger.error("调用sys.call.conf_exit失败call_id={},result={}",call_id,rpcError);
                }
