 ...
 */
public class TemplatePassedEventHandler implements MQMessageHandler<TemplatePassedEvent> {
    ...
    @Override
    public void handleMessage(TemplatePassedEvent message) throws JMSException {
        String tempId = message.getTempId();
        MsgTemplate template = msgTemplateService.findByTempId(tempId);
        if(MsgTemplate.STATUS_PASS != template.getStatus()){
            return;
        }
        ...
        if(StringUtils.isBlank(callbackUrl)){
            Map<String,Object> notify_data = new MapBuilder<String,Object>()
                    .putIfNotEmpty("event","msg.template_passed")
