        ...
        if(list != null){
            for (int i = 0; i < list.size(); i++) {
                ...
                if(CertAccountQuotaType.AgentQuota.name().equals( certAccountQuota.getType() )){
                    this.seatNum = ( null == certAccountQuota.getCurrentUsed() ? "0": certAccountQuota.getValue() ) +"/" + ( -1 == certAccountQuota.getValue() ? "∞": certAccountQuota.getValue() );
                }else if(CertAccountQuotaType.CallQuota.name().equals( certAccountQuota.getType())){
                    this.voiceNum = ( null == certAccountQuota.getCurrentUsed() ? "0": certAccountQuota.getValue() )+"/" + ( -1 == certAccountQuota.getValue() ? "∞": certAccountQuota.getValue() );
                }
            }
        }
    }
