        ...
        if(list != null){
            for (int i = 0; i < list.size(); i++) {
                ...
                if(CertAccountQuotaType.AgentQuota.name().equals( certAccountQuota.getType() )){
                    this.seatNum = certAccountQuota.getUsed() +"/" + ( -1 == certAccountQuota.getValue() ? "∞": certAccountQuota.getValue() );
                }else if(CertAccountQuotaType.CallQuota.name().equals( certAccountQuota.getType())){
                    this.voiceNum = certAccountQuota.getUsed() +"/" + ( -1 == certAccountQuota.getValue() ? "∞": certAccountQuota.getValue() );
                }
            }
        }
    }
