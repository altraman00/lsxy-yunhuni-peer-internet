...
public class ClassPathScanUtil {  
     ...
     * logger 
     */  
    private static final Log logger = LogFactory.getLog(ClassPathScanUtil.class);
     ...
     * @return Set 
     */  
    public Set<Class<?>> getPackageAllClasses(String basePackage, boolean recursive,Vector<URL> dirs) {  
        ...
        Enumeration<URL> dirsEnum;  
        
        try {  
        	if(dirs == null){
            	dirsEnum = Thread.currentThread().getContextClassLoader().getResources(package2Path);  
            }else{
            	dirsEnum = dirs.elements();
            }
            while (dirsEnum.hasMoreElements()) {  
                URL url = dirsEnum.nextElement();  
    ...
    private void doScanPackageClassesByJar(String basePackage, URL url,  
        ...
        try {  
            ...
            logger.error("指定的jar读取失败:"+url.toString());
        }  
    }  
    ...
    public static void main(String[] args) {  
        ...
        calssList = handler.getPackageAllClasses("michael.hessian", true,null);  
        for (Class<?> cla : calssList) {  
            System.out.println(cla.getName());  
        }  
    }  
